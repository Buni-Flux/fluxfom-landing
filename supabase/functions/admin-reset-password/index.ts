...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Must use the anon (publishable) key with the caller's JWT — service role + getUser() yields no user.
    ...
    // Some clients/gateways drop `action`; infer from payload shape + headers.
    ...
    if (action === "reset_password") {
      ...
      // Get user email (recovery links require an email; phone-only / some OAuth users have none)
    ...
    return new Response(
      JSON.stringify({
        error:
          ...
          "Deploy admin-manage-users or admin-reset-password from this repo to the Supabase project matching VITE_SUPABASE_URL.",
      }),
      {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
